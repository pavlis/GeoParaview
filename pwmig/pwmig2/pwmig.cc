...
/* This function gets the TimeSeries or ThreeComponentSeismogram
of coherence estimates matching a given seismogram. dbh is assumed
to be a match handle keyed by pwfid and paramname.  It is also assumed
the parent against which it is matched is a view of wfprocess limited
to coherence traces.  It is assumed this view is a join of attrlink->wfprocess
with the view limited to coherence traces only.  

This algorithm is fussy and will abort the program if anything goes wrong.
This was done because something has to have been done drastically wrong
if the conditions here are not met and it is appropriate for the program
to abort if they occur.  

Arguments:
	dbh - database handle as described above
	d - data trace for which a coherence trace match is desired in dbh
	mdl - metadata list to pass to Tseis constructor (bare minimum is all that
		is required)
	am - attribute map required by Tseis constructor.

Returns a pointer to a Tseis object.  In practice this means either a 
ThreeComponentSeismogram or a TimeSeriesObject.  I used a template to 
avoid the nearly identical code that would have been required otherwise.
The return is a set of coherence estimates for the data produced by pwstack.
The 3C version is computed component by component and the scalar version is
one coherence figure for all 3 components.

I chose a pointer for efficiency and to avoid some initialization issues
created by making coherence weighting optional.
*/
template <class Tseis> Tseis LoadCohWeights(DatascopeMatchHandle dbh,
	ThreeComponentSeismogram& d, MetadataList& mdl, AttributeMap& am)
{
	list<int> record;
	list<int>::iterator irec;
	const string dtype3c("3c");
	try {
		// the attrlink table is more general.  For now we always match
		// this keyword so we force it here.
		d.put(string("paramname"),string("coherence"));

		record=dbh.find(dynamic_cast<Metadata&>(d));
		
	} catch (...)
	{
		cerr << "LoadCohWeights:  DatascopeMatchHandle failure in find function"<<endl
			<< "Match key is pwfid"<<endl
			<< "Metadata contents against which match was attempted"<<endl
			<< dynamic_cast<Metadata&>(d)
			<< endl  << "Exiting"  <<endl;
		exit(-1);
	}
	int rec_scalar=-1,rec3c=-1;
	Dbptr db=dbh.db;
	char cbuf[4];
	for(irec=record.begin();irec!=record.end();++irec)
	{
		db.record=*irec;
		if(dbgetv(db,0,"datatype",cbuf,0) == dbINVALID)
		{
			cerr << "LoadCohWeights:  dbgetv error reading datatype"
				<< endl << "Cannot continue:  exiting"<<endl;
			exit(-1);
		}
		if(string(cbuf)==dtype3c)
			rec3c=*irec;
		else
			rec_scalar=*irec;
	}
	//Stroustrup says this kind of construct is evil, but is a useful
	// construct in this case even if it isn't that efficient.
	if(typeid(Tseis)==typeid(ThreeComponentSeismogram))
	{
		if(rec3c<0)
		{
			cerr << "LoadCohWeights:  required coherence 3C attribute seismogram "
		...
		// here and in the else block below we ASSUME db is a join of
		// wfprocess and the attrlink table using the attribute 
		// waveform id as the join key to attrlink.  That provides
		// a view that is only wfprocess attribute traces a no
		// seismic data.  This line will fail if this isn't true.
		// Should be ok in this code, but DO NOT use this template
		// in another program without heeding this warning
		dbh.db.record=rec3c;
	}
	else
	{
		if(rec_scalar<0)
		{
			cerr << "LoadCohWeights:  required coherence scalar attribute seismogram "
				<< "not found in database.  "
				<< endl
				<< "Cannot continue.  Exiting"
				<< endl;
		...
		dbh.db.record=rec_scalar;	}
	try {
		Tseis result(dynamic_cast<DatabaseHandle&>(dbh),mdl,am);
		return(result);
	} catch(SeisppError serr)
	{
		cerr << "LoadCohWeights:  Fatal error"<<endl
			<< "Failure in database constructor for coherence trace object"<<endl
...
// good for teleseismic data and is unlikely to be a big problem.
		...
		MetadataList mdlcoh=pfget_mdlist(pf,"Coherence_trace_mdlist");
		...
		dbh.group(groupkey);
		dbh.rewind();
		/* We'll always build this match handle even if this
		feature is not enabled.  Not a large  overhead to create
		it and this avoids some initialization issues.  This 
		requires first building a (frozen) join between
		the attrlink an wfprocess tables joined by the waveform
		id of the attribute trace data.  */
		DatascopeHandle dbhcohp(dbh); 
		dbhcohp.lookup("wfprocess");
		list<string> mkl,mkr;
		mkr.push_back(string("pwfid"));
		mkl.push_back(string("attwfid"));
		// VERY important we join wfprocess on the right so
		// pwfid without the qualifier is the one from attrlink
		dbhcohp.join(string("attrlink"),string("wfprocess"),
			mkl,mkr);
		// Slighly wasteful of memory here but small compared
		// to some things later
		list<string> matchlist;
		matchlist.push_back(string("pwfid"));
		// Not yet defined for test data set but needed
		//matchlist.push_back(string("paramname"));
		DatascopeMatchHandle dbhcoh(dbhcohp,string(""),
			matchlist,am);
double rundtime;
rundtime=now();
...
mp.process(string("figure(1);plot6c(de,sptime,di);pause(1.0);"));
				...
				if(use_coh_weights)
				{
					...
					{
						ThreeComponentSeismogram coh3cwt
						  =LoadCohWeights<ThreeComponentSeismogram>
							(dbhcoh,
							pwdata->member[is],
							mdlcoh,am);
						dmatrix cwt(3,nout);
