...
#include "DatascopeProcessingQueue.h"
using namespace std;
...
/* Added June 2009 to implement next method that is part of virtual base for
this beast.  
*/
int DatascopeProcessingQueue::next(DatabaseHandle& dbhbase)
{
	const string base_error("DatascopeProcessingQueue::next() method: ");
	DatascopeHandle dbh;
	try {
		dbh=dynamic_cast<DatascopeHandle&>(dbhbase);
	} catch(std::exception& e)
	{
		string message;
		message=base_error+"dynamic_cast to DatascopeHandle failed.\nstdlib error messaage:  "
				+ e.what();
		throw SeisppError(message);
	}
	int newrecord;
	try {
		newrecord=this->next(dbh);
	} catch (SeisppError serr){throw serr;};
	return(newrecord);
}
int DatascopeProcessingQueue::next(DatascopeHandle& dbh)
{
	const string base_error("DatascopeProcessingQueue::next() method: ");
	if(dbh.db.record<0)
	{
		throw SeisppError(base_error 
			+ "Illegal db.record value (negative).  Probable initialization error.");
	}
	int newrecord;
	newrecord=this->position_to_next(dbh.db.record);
	if(newrecord<0)
		AtEOF=true;
	dbh.db.record=newrecord;
	return(newrecord);
}
...
void DatascopeProcessingQueue::mark(ProcessingStatus pstat)
{
	...
	if(lockf(fd,F_ULOCK,(off_t)0))
		throw SeisppError(string("DatascopeProcessingQueue::mark:  ")
		  + "lockf failed when attempting to release lock on queue file");
}
